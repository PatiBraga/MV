    ...
    WHEN CAST(:INDICADOR AS VARCHAR(100)) = 'Custo da Folha' then
        case
           when "TIPO_CONTRATACAO" in ('EMPREGADO','APRENDIZ') THEN COALESCE("VL_PROVENTO",0)
                                                                             ...
                                                                             + COALESCE("VL_FGTS",0)
                                                                             + ((COALESCE("VL_PROVENTO",0) + COALESCE("VL_VANTAGEM",0)) * 0.2)  --Proventos, vale transporte,  plano de saude,  fgts e INSS empresa
...
from
  %EVENTOS%
WHERE
  CAST(:INDICADOR AS VARCHAR(100)) IN ('Todos os Eventos','Salário Líquido','ATS','Insalubridade','Provento','Desconto','Gratificações'
