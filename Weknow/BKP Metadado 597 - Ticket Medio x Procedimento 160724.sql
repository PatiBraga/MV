...
From
    ...
    d.cd_aviso_cirurgia,
    g.CD_PRO_FAT cd_pro_fat_cir,
    g.DS_PRO_FAT ds_pro_fat_cir,
    f.ds_cirurgia,
    'Internado' origem,
    ...
    and itreg_fat.cd_mvto = c.cd_mvto_estoque (+)
    and itreg_fat.cd_itmvto = c.cd_itmvto_estoque (+)
    and c.cd_mvto_estoque = d.cd_mvto_estoque (+)
    and d.cd_aviso_cirurgia = e.cd_aviso_cirurgia (+)
    and e.cd_cirurgia = f.cd_cirurgia (+)
    and f.cd_pro_fat = g.cd_pro_fat (+)
	Group by
    ...
    and itreg_amb.cd_mvto = c.cd_mvto_estoque (+)
    and itreg_amb.cd_itmvto = c.cd_itmvto_estoque (+)
    and c.cd_mvto_estoque = d.cd_mvto_estoque (+)
    and d.cd_aviso_cirurgia = e.cd_aviso_cirurgia (+)
    and e.cd_cirurgia = f.cd_cirurgia (+)
    and f.cd_pro_fat = g.cd_pro_fat (+)
