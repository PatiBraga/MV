SELECT DISTINCT
      ...
      cd_usuario,
       nm_usuario,
       (select LISTAGG(ds_centro_custo, ', ') WITHIN GROUP (ORDER BY 1) from COLABORADORES_ATIVO_MARCELO where ds_colaborador = nm_usuario)setor
                ...
                (
                 ...
                 WHERE
                       --CD_USUARIO IN ('FERNANDA.SLIMA')
                        TELA LIKE 'M_CONSULTA_PACIENTE'
                       --AND MODULO LIKE '%LABORATO%'
                        --cd_papel = 157
                       --AND CD_USUARIO IN(SELECT CD_ID_USUARIO FROM USUARIO_MULTI_EMPRESA WHERE CD_MULTI_EMPRESA = 3)
                       and (select LISTAGG(ds_centro_custo, ', ') WITHIN GROUP (ORDER BY 1) from COLABORADORES_ATIVO_MARCELO where ds_colaborador = nm_usuario) is not null